...
/**
 * Servlet implementation class DelayQueueOnboarding
 */
@WebServlet("/DelayQueueOnboarding")
public class DelayQueueOnboarding extends HttpServlet {
				...
				if(!UserActivity.isContainValue(cronData.getQueueName())) {
					...
					if (cronData.getTimeUnit().equals("SECONDS") || cronData.getTimeUnit().equals("MINUTES") || cronData.getTimeUnit().equals("HOURS")) {
						...
						if(cronData.getTimeUnit().equals("SECONDS")) {
							...
							userActivity.addDelayQueueToRedis(cronData.getDelayTime(), TimeUnit.MINUTES, cronData.getData(), cronData.getQueueName());
						} else {
							userActivity.addDelayQueueToRedis(cronData.getDelayTime(), TimeUnit.HOURS, cronData.getData(), cronData.getQueueName());
						}
					} else {
